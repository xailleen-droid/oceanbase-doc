...
例如，在开发 RAG 应用时，我们通常需要将文本数据进行嵌入处理转换为向量之后存储在向量数据库中，而其他结构化数据存储在关系型数据库中。

OceanBase 4.3.3 版本开始支持将向量作为一种字段类型在关系表中进行存储，使得向量和传统标量数据能够有序、高效地存储在 OceanBase 这一款数据库中。
