...
* `WITH PARSER {SPACE | NGRAM | BENG}`：可选，指定全文索引的分词器。当前版本仅支持 `SPACE`（空格）、`NGRAM` 和 `BENG`（基础英文）三种内置分词器，不指定默认使用 `SPACE`（空格）分词器。可以使用 [TOKENIZE](../../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/400.functions-of-mysql-mode/960.text-processing-functions-of-mysql-mode/200.tokenize-of-mysql-mode.md) 函数查看文本按照指定的分词器及 Json 形式参数的分词结果。

* `LOCAL`：可选，指定创建局部索引。

   <main id="notice" type='explain'>
     <h4>说明</h4>
     <p>当创建索引时，如果没有指定 <code>LOCAL</code> 或者 <code>GLOBAL</code> 关键字，在分区表上会默认会创建出局部索引（<code>LOCAL</code>）。</p>
   </main>
