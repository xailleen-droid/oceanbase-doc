...
> 说明
  上述代码中 cpu_count 表示创建租户时指定的 min_cpu。
...
> 说明
  alter table t1 drop column group(all columns, each column); 执行后，不用担心没有任何 Group 来承载数据，所有列会被放到一个叫做 DEFAUTL COLUMN GROUP 的默认 Group 中。
