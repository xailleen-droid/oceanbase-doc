...
3. 配置环境变量：
   ...
   将 username、password、host、port 和 database 替换为你的 OceanBase 数据库连接信息
   确保 schema 参数与你的数据库架构名称匹配
   如果使用 SSL 连接，需要添加 ?sslmode=require 参数
...
6. 创建数据库迁移（仅适用于新项目）：
   ...
   * 重新生成 Prisma 客户端
   ...
   > 注意
   如果已经使用 db pull 从现有数据库拉取了表结构，则不需要执行此步骤。
