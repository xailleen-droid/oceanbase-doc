...
> 注意
   ...
   将 host、port、username、password 和 database 替换为你的 OceanBase 数据库连接信息
   如果使用 SSL 连接，需要添加 ssl: { rejectUnauthorized: false } 配置
   开发时可以将 synchronize 设为 true，但在生产环境建议使用迁移（migrations）来管理数据库结构变更
   建议使用 .env 文件和环境变量来存储敏感信息
