...
4. 编写 Shell 脚本逻辑。
    ...
    **示例如下：**
    ...
    > 说明
      该脚本将生成一个名为 insert_test_sql_file_tbl1.sql 的 SQL 文件，并在其中插入了 100,000 条用户记录。您可以根据需要修改 SQL 语句和循环生成的用户记录数量。在进行大量数据插入时，请提前关注相关服务器的资源使用情况，避免由于资源使用情况而导致数据插入失败或性能下降的情况。
...
> 说明
  更多有关执行 SQL 脚本的信息，请参见 从 SQL 文件导入数据到 OceanBase 数据库。
...
* `$database_name`：需要访问的数据库名称。
    ...
    > 注意
        连接租户的用户需要拥有该数据库的 CREATE、INSERT 和 SELECT 权限。更多有关用户权限的信息，请参见 MySQL 模式下的权限分类。
