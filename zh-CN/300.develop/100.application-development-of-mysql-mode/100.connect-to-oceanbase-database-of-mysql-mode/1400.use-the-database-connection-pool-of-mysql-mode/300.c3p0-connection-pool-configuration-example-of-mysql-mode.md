...
2. 根据 OceanBase 数据库连接串信息填写下面 URL 的对应信息。
    ...
    * `$database_name`：需要访问的数据库名称。
        ...
        > 注意
          连接租户的用户需要拥有该数据库的 CREATE、INSERT、DELETE、UPDATE 和 SELECT 权限。更多有关用户权限的信息，请参见 MySQL 模式下的权限分类。
...
5. 配置项目所依赖组件。
    ...
    > 说明
      本部分代码定义项目所依赖组件是 MySQL Connector/J 的 8.0.25 版本，如果需要了解其他版本的信息，请参见 MySQL Connector/J。
...
3. 配置数据库驱动。
    ...
    > 说明
      有关 MySQL Connector/J 实现类的名称信息，请参见 Driver/Datasource Class Name。
