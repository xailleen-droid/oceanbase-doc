...
> 说明
  本文主要介绍使用 CREATE INDEX 语句创建索引，多值索引除外。多值索引仅支持在创建表时创建。其他创建索引的方式请参见 CREATE TABLE 或 ALTER TABLE 语句。
...
多值索引是一种特殊的数据库索引，用于处理在同一列中可以存储多个值的情况。OceanBase 数据库支持在创建表时（`CREATE TABLE`）来预先创建多值索引。目前不支持在表创建之后添加多值索引，但可以通过 `ALTER TABLE` 语句来删除已存在的索引。对于已创建包含多值索引的主表，支持了复杂 DML 操作。
