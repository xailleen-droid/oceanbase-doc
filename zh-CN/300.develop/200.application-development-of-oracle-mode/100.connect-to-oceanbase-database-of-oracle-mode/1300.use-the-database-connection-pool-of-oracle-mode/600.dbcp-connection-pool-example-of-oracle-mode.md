...
### 步骤一：将 dbcp-oceanbase-client 项目导入到 Eclipse 中
...
<table>
   ...
   <tr>
       ...
       <td>N/A</td>
       <td>指定以键值对形式的额外连接属性，这些属性将在获取数据库连接时传递给底层的 JDBC驱动程序。字符串的格式必须是 <code>propertyName=property;</code> > 注意username 和 password 属性将被明确传递，因此不需要在此处包含它们。</td>
   </tr>
   <tr>
