...
> **注意**
>
> 本文中的参数介绍信息基于 OceanBase 数据库 V4.3.5 BP1 版本中的配置模板更新。若需查询其他版本的推荐配置，请通过对应版本安装包中的配置文件获取最新参数组合。
...
> 说明
  关于在线事务处理（EXPRESS_OLTP）场景下配置推荐的更详细介绍，参见 在线事务处理场景配置最佳实践。
...
> 说明
  关于复杂联机事务处理（COMPLEX OLTP）场景下配置推荐的更详细介绍，参见 复杂联机事务处理场景配置最佳实践。
...
> 说明
  关于实时分析（OLAP）场景下配置推荐的更详细介绍，参见 实时分析场景配置最佳实践。
...
| default_table_organization | HEAP | 表中数据行的存储顺序。对于 AP 场景，表中的数据行没有特定的顺序存储。> **说明**
>
> 该配置项从 V4.3.5 BP1 版本开始引入。| tenant |
...
> 说明
  关于混合事务/分析处理（以下使用 HTAP）场景下配置推荐的更详细介绍，参见 混合事务/分析处理场景配置最佳实践。
...
> 说明
  关于 OBKV 在不同业务场景下配置推荐的更详细介绍，参见 OBKV 配置最佳实践。
