...
1. （可选）修改配置文件
   ...
   > 说明
     ...
     目前 OBServer 节点的 Zone（默认为 zone1）不支持修改。
   ...
   | redo_dir  | 可选      | /var/lib/oceanbase/redo       | clog、ilog 和 slog 的目录，建议配置为独立的磁盘。> 说明数据盘（默认为 /var/lib/oceanbase/）的可用磁盘空间应当大于 15G。 |
   | data_dir  | 可选      | /var/lib/oceanbase/data       | 设置存储 SSTable 等数据的目录，建议配置为独立的磁盘。> 说明数据盘（默认为 /var/lib/oceanbase/）的可用磁盘空间应当大于 15G。 |
