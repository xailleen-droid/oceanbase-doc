...
> 说明
   本节以 OAT V4.2.0 版本为例提供操作指导。不同 OAT 版本的操作界面可能不同，请以实际界面为准。推荐在 OceanBase 中创建独立的租户用于 MetaDB。MetaDB 用于存储除了监控数据之外的其它持久化数据，推荐分配 CPU 和内存不少于 4 C，8 GB。磁盘容量受 OCP 管控的集群规模、存储的文件资源和数据保留时间等因素的影响，一般推荐磁盘有 100 GB 以上。
...
2. 在 **创建 MetaDB** 页面，配置 **基础配置**。
    ...
    | 自定义 Zone | 开启自定义 Zone，需要填写 Zone 名称、机房、服务器、Root Server 位置。> **注意**
>
> 此参数与服务器参数二选一，当打开自定义 Zone 后，上述服务器参数会自动隐藏。|
    | 配置仲裁服务 | 可自由开启或者关闭，开启时需要填写仲裁服务地址，示例：xxx.xxx.xxx.xxx:xxxx。> **注意**
>
> 当 MetaDB 镜像处选择 V4.X 版本的 MetaDB 时，才能配置仲裁服务。|
    | CPU | MetaDB 在服务器上可用的 CPU，默认为 `32C`。 > **说明**
>
> 如果您准备的 OCP 服务器 CPU 和 内存 是 16 C 32 GB 的配置，建议您设置 MetaDB 在服务器上可用的 CPU 为 8 C。|
    | 内存 | MetaDB 在服务器上可用的内存，默认为 `90GB`。 > **说明**
>
> 如果您准备的 OCP 服务器 CPU 和 内存 是 16 C 32 GB 的配置，建议您设置 MetaDB 在服务器上可用的内存为 16 GB。|
...
4. 在 **创建 MetaDB** 页面，配置 **启动参数（选填）**。
    ...
    > 注意
      如果您准备的 OCP 服务器 CPU 和 内存 是 16 C 32 GB 的配置，并且您设置 MetaDB 在服务器上可用的 CPU 和内存是 8 C 16 GB，您需要在 observer 进程启动参数 中设置参数 system_memory 的值为 5G，参数 resource_hard_limit 的值为 150。
