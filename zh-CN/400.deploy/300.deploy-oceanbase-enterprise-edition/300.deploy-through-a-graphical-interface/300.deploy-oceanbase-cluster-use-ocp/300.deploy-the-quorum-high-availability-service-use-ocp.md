...
> 说明
   本节以 OCP V4.2.1 版本为例提供操作指导。不同 OCP 版本的操作界面可能不同，请以实际界面为准。单个 OceanBase 集群只能添加一个仲裁服务。
...
4. 在 **新建仲裁服务** 面板中，设置仲裁服务的基础信息。
   ...
   | 参数 | 说明 |
   | ------ | ------ |
   | 主机 | 选择部署仲裁服务的主机，同一台主机可以启动多个仲裁服务进程。支持根据 IP 搜索主机，同时支持在下拉栏中单击 **添加主机** 来新增可用主机。> **注意**
>
> 不支持选择已部署 OceanBase 服务的主机。|
   ...
   | 参数设置 | 设置仲裁服务启动参数。 > 注意为了确保仲裁节点的日志目录能自动清理并防止磁盘写满，需要将 enable_syslog_recycle 设置为 True，并且将 max_syslog_file_count 设置为一个有效值。否则，日志文件会不断增长，可能导致磁盘空间耗尽。|
