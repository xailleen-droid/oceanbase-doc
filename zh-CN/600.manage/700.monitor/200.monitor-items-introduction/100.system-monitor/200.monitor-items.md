...
| sql local execute time | 本地数据库节点执行 SQL 查询的耗时|
| sql remote execute time | 在远程节点执行 SQL 查询的耗时|
| sql distributed execute time | 布式事务或跨多节点协同执行 SQL 的总耗时|
...
| xa start total used time       |`xa_start` 语句的总耗时（包含成功的以及失败的）  |
...
| xa end total used count        | `xa_end` 语句的总耗时，包含成功的与失败的       |
...
| xa prepare total used time     | `xa_prepare` 语句的总耗时（包含成功的与失败的） |
...
| xa commit total used time      | `xa_commit` 语句的总耗时（包含成功的与失败的）   |
...
| xa rollback total used time    | `xa_rollback` 语句的总耗时（包含成功的与失败的）  |
...
| application wait total time            | 由用户应用程序代码产生的等待时间总和（例如，由行级锁定或显式锁定命令引起的锁等待）|
