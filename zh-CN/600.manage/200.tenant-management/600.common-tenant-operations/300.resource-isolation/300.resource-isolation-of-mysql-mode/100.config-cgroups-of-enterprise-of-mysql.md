...
cgroup 系统目录的配置操作可以在 OceanBase 数据库软件安装前进行，也可以在 OceanBase 数据库软件安装后进行。本文档以 OceanBase 数据库软件已安装的前提下为例，提供 cgroup 系统目录的配置操作指导。
配置 cgroup 系统目录时需要 root 用户权限。
...
2. 执行以下命令，挂载 `/sys/fs/cgroup` 目录。
   ...
   > 说明
   如果已经存在 /sys/fs/cgroup 目录，则可忽略该步骤。
...
3. 创建 `/sys/fs/cgroup/cpu` 目录并修改 Owner 后，挂载 cpu 子系统。
   ...
   > 说明
   如果已经存在 /sys/fs/cgroup/cpu 目录且目录为空，则可忽略该步骤。
...
3. 创建 `/sys/fs/cgroup/cpu` 目录并修改 Owner 后，挂载 CPU 子系统。
   ...
   > 说明
   如果已经存在 /sys/fs/cgroup/cpu 目录，则可忽略该步骤。
...
建议使用 cgroup 中的目录文件调用 `DBMS_RESOURCE_MANAGER` 系统包中的子程序 `CREATE_CONSUMER_GROUP` 创建的资源组，来进行资源隔离。配置租户内资源隔离的详细操作，参见 [配置租户内资源隔离](200.resource-isolation-at-user-level-of-mysql-mode.md)。
