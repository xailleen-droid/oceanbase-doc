...
> **说明**
>
> 对于基于日志归档的物理备库场景，不会出现以上报错，需要您参考以下步骤自行检查归档源所在的租户是否已经切换为备租户，否则可能会出现双主租户或者本次 Switchover 操作一直无法完成日志同步的问题。
...
1. 执行以下 SQL，查询当前租户 `SWITCHOVER_STATUS` 的状态。
    ...
    有关 `SWITCHOVER_STATUS` 字段中各状态的详细说明，请参见 [DBA_OB_TENANTS](../../../700.reference/700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/5800.oceanbase-dba_ob_tenants-of-sys-tenant.md)。
...
> 注意
报错信息 original error code: -4012 中的  -4012，仅代表其中一种可能出现的错误码，该报错还有可能显示为其它错误码。
...
> 说明
执行 SWITCHOVER TO STANDBY VERIFY 命令时，系统仅检查备租户与恢复源的系统日志流是否同步；执行 SWITCHOVER TO STANDBY命令时，系统会检查备租户与恢复源的所有日志流是否同步。
