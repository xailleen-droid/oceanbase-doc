...
> **注意**
>
> OceanBase 数据库暂不支持 V4.2.x 系列或更低版本升级到 V4.3.x 版本。
...
> 注意
  如果 OceanBase 集群关联了仲裁服务，那么在进行 OceanBase 集群版本升级时，需要确保先升级仲裁服务版本，然后再升级 OceanBase 集群版本。仲裁服务版本升级的详细信息，请参见 升级仲裁服务。可以使用系统租户查询视图 oceanbase.DBA_OB_ARBITRATION_SERVICE 检查本 OceanBase 集群是否关联仲裁服务。
...
> 说明
  本文升级路径图并未包含 OceanBase 数据库的所有版本，如需了解更多版本的升级路径，可以参考 oceanbase_upgrade_dep.yml 文件，该文件记录了 OceanBase 的版本升级拓扑图。解压目标版本的 OceanBase RPM 包后，会在存放 RPM 包的目录下生成 home 和 use 两个目录。升级相关脚本和 oceanbase_upgrade_dep.yml 文件存放 home/admin/oceanbase/etc 目录下。使用 oceanbase_upgrade_dep.yml 文件确认升级路径的详细信息，参见 升级 OceanBase 集群 中的 步骤二：确认升级流程 章节。
