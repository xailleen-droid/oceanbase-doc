...
* OceanBase 数据库 MySQL Mode 一般性规范
    ...
    * 不以 OceanBase 数据库 MySQL Mode 中存在的关键字、保留字命名。
    ...
    > **注意**
>
> OceanBase 数据库 MySQL Mode 当前表名最大长度限制为 64，具体版本可能有不同限制。
...
* OceanBase 数据库 Oracle Mode 一般性规范
    ...
    * 不以 OceanBase 数据库 Oracle Mode 中存在的关键字、保留字命名。
    ...
    > 注意
    OceanBase 数据库 Oracle Mode 规范除字母大小写要求之外，其余与 OceanBase 数据库 MySQL Mode规范相同。OceanBase 数据库 Oracle Mode 当前表名最大长度限制为 64，具体版本可能有不同限制。
...
* 临时表
    ...
    **补充场景说明**
    ...
    > 注意
    歧义指资源命名虽然符合规范，但有多种解释空间，如以 tmp_ 开头的表作为普通表，该表又可被视为临时表，应当避免此类情况。
...
* OceanBase 数据库 MySQL Mode 索引命名规范
    ...
    * 分库分表同一逻辑表的不同物理分表的索引名应当保持一致，不做分表位的区分。
    ...
    > **注意**
>
> 当前 MySQL、OceanBase 数据库 MySQL Mode 主流版本索引长度限制为 64 个字符，不同版本可能存在差异。
...
* Oracle、OceanBase 数据库 Oracle Mode 索引命名规范
    ...
    * 索引名由有业务含义的字符描述，建议以所包含的列名或列名的适当形式缩写加分隔符 `_` 组成。
    ...
    > 注意
    当前 OceanBase 数据库 Oracle Mode 主流版本索引长度限制为 64 个字符，不同版本可能存在差异。分库分表的同一个逻辑表内的不同物理分表的索引命名相同，仅能在分库分表数相同场景实现，如 10 库 10 表，100 库 100 表，在 10 库 100 内会造成冲突。分库分表的同一个逻辑表内的不同物理分表的索引命名以分表位区分，指索引中包含 00-09 或 000-099 此类分表位，用于避免同一 SCHEMA 内索引命名冲突限制。分库分表的同一个逻辑表内的不同物理分表的索引命名以表名区分，指索引中包含物理分表信息，因物理分表中包含了分表位信息，从而避免了同一 SCHEMA 内索引命名冲突。如一个分库分表的同一个逻辑表内不同物理分表的索引命名不同，可能在使用 hint 上有所限制。hint 中的指定索引信息不能进行分表位解析处理，所以如业务需求必须在 hint 中使用索引，应当使用分库分表数量相同的逻辑库。
