...
* 审计日志
    ...
    > 说明
    query_digest_time_threshold 用于设置请求执行时间阈值，超过阈值即打印一条日志到 obproxy_digest.log 文件，取值范围为 [0s, 30d]。
...
* 慢日志
    ...
    > 说明
    slow_query_time_threshold 慢请求执行时间阈值。超过阈值即打印一条日志到 obproxy_slow.log 文件，取值范围为 [0s, 30d]。
