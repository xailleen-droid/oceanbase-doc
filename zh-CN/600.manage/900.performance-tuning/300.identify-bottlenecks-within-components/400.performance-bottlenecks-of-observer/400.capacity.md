...
* 调整集群参数
    ...
    > 说明
    cpu_quota_concurrency用于设置租户的每个 CPU 配额所允许的最大并发数。详细信息，参见 cpu_quota_concurrency。

* SQL 限流

> 注意
    当出现容量问题时，一般采用扩容的方式解决（或者是调大租户的 CPU 规格，或者是调大 cpu_quota_concurrency），工作线程增加后需要使用的内存资源也会增加，所以还需要同步调大租户的内存规格。
