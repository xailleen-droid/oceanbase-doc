...
OceanBase 数据库当前版本支持阿里云 OSS、NFS、AWS S3 以及兼容 S3 协议的对象存储（例如：华为 OBS、Google GCS、腾讯云 COS） 等备份介质，部分备份介质需要满足一些基本要求才能使用。
...
> **说明**
>
> 尽管 S3 及以兼容 S3 协议访问的备份介质与 OSS、NFS、COS 等备份介质的日志归档目录结构不相同，但将这些备份介质上的备份文件通过跨云拷贝到 OSS、NFS、COS 等备份介质上后，仍然支持恢复。例如，将原生 S3 归档的数据拷贝到 OSS 上，使用该 OSS 路径，仍然可以成功恢复。
