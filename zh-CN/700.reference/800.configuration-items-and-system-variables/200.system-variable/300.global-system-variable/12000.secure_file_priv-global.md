...
|-----------|---------|
| 参数类型    | Varchar |
| 默认值      | ""，表示空字符串。> 说明从 V4.2.1 版本开始默认值由 NULL 调整为 ""。  |
| 取值范围    | <ul><li>空字符串：表示导入、导出被禁用 </li><li> `NULL`：表示导入、导出被禁用  </li><li> `"NULL"`：表示导入、导出被禁用  </li><li>路径名：表示仅该路径下的文件可以被导入或导出  </li><li>`"/"`：表示没有限制，任意路径均可访问 </li></ul>> 说明从 V4.2.0 版本开始，枚举值空字符串和 NULL 的含义由原来的无限制调整为导入、导出被禁用。   |
| 生效范围    | GLOBAL  |
