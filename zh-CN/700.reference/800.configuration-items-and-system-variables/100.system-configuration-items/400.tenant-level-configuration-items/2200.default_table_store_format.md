...
| 参数类型 | String |
| 默认值 | row |
| 取值范围 | ("row", "column", "compound")> 说明row：用户租户建表的默认格式为行存。  column：若用户租户建表语句后未加 with column group 语句，则会自动加上 with column group(each column) 语句，默认格式指定为纯列存表compound：若用户租户建表语句后未加 with column group 语句，则会自动加上 with column group(all columns, each column) 语句，默认格式指定为冗余行存列存表。|
