...
|是否重启 OBServer 节点生效|否，设置后立即生效。> 说明由于 JVM 启动参数属于运行时环境基础配置，其加载机制遵循 Java 虚拟机规范：参数初始化发生在 JVM 启动阶段。禁止运行时动态重载（Runtime Reloading）。不支持热更新（Hot-swapping）机制。|
...
> 注意
  请确保 /home/observer/heapdumps 和 /home/observer 目录存在。如果这些目录不存在，需要使用 OBServer 的部署用户手动创建。请注意，这里的路径仅为示例，您可以根据实际情况指定其他路径。
