...
* [OceanBase 数据库日志 ob_px_admission 报错 out of px worker resource](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000209995)
  ...
  > 说明
    该错误码在 V4.0.0 版本开始进行了修改，错误信息由 ERROR 5307 (HY000) : Scheduler thread number is not enough  调整为 ERROR 5307 (HY000) : Not enough thread resource to execute the query. Require setting parallel_servers_target to a value no less than %ld for this query 。
...
> 说明
  该错误码从 V4.3.0 版本开始报错信息由 oci lib not founded 调整为 can not find the expected version of OCI LIB: %.*s。
