...
3. 设置租户的最大连接数。
   ...
   * 设置租户内普通用户总的最大并发连接数
     ...
     > 说明
     如果当前租户同时设置了_resource_limit_max_session_num、max_user_connections 和 max_connections 的值，则对于某个普通用户，其并发连接数只要达到其中一个值，即无法再建立新的连接；对于管理员用户，其并发连接数只要达到 max_user_connections 或 max_connections 的其中一个值，即无法再建立新的连接。
