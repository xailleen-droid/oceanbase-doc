...
| -u/--unit | 否 | string | 无 | 租户资源池使用的资源规格。> 说明可通过如 --zone1.unit=s1 的方式单独指定某个 Zone 上的资源规格，该方式的优先级高于通过 -u/--unit 指定。 |
| --unit_num | 否 | int | 1 | 租户在目标 Zone 上的 unit 个数，它的值要小于目标 Zone 的节点个数。 |
| --replica_type  | 否 | string | FULL | 租户在目标 Zone 上的副本类型，可取值为 `FULL`（全功能型副本）或者 `READONLY`（只读型副本）。> 说明可通过如 --zone1.replica_type=FULL 的方式单独指定某个 Zone 上的副本类型，该方式的优先级高于通过 --replica_type 指定。 |
...
| --whitelist | 否 | string | 无 | 指定租户的访问白名单。 |
| --password | 否 | 无 | 无 | 指定是否采用交互的方式修改 root 用户密码，无需传值。> 说明该选项与 --old_password/--new_password 互斥，不可同时指定，同时指定时会报错。 |
...
| -u/--unit | 否 | string | 无 | 租户资源池使用的资源规格。> 说明可通过如 --zone1.unit=s1 的方式单独指定某个 Zone 上的资源规格，该方式的优先级高于通过 -u/--unit 指定。 |
| --replica_type | 否 | string | 无 | 租户在目标 Zone 上的副本类型，可配置为 `FULL`（全功能型副本）或者 `READONLY`（只读型副本）。> 说明可通过如 --zone1.replica_type=FULL 的方式单独指定某个 Zone 上的副本类型，该方式的优先级高于通过 --replica_type 指定。 |
...
| -u/--unit | 否 | string | 无 | 租户资源池使用的资源规格。> 说明可通过如 --zone1.unit=s1 的方式单独指定某个 Zone 上的资源规格，该方式的优先级高于通过 -u/--unit 指定。 |
| --replica_type  | 否 | string | FULL | 租户在目标 Zone 上的副本类型，可设置为 `FULL`（全功能型副本）或者 `READONLY`（只读型副本）。> 说明可通过如 --zone1.replica_type=FULL 的方式单独指定某个 Zone 上的副本类型，该方式的优先级高于通过 --replica_type 指定。 |
| --primary_zone | 否 | string | RANDOM | 租户的 Primary Zone，指定了租户提供读写服务的 Zone 的优先级。例如 `primary_zone ='zone1;zone2,zone3'` 表示该租户优先由 `zone1` 提供读写服务，`zone1` 比 `zone2`、`zone3` 的优先级高，`zone2` 和 `zone3` 是同一优先级。 |
