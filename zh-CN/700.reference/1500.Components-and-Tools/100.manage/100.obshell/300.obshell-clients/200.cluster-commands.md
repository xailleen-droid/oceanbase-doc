...
| -I/--immediate | 否  |  | 无  | 控制是否执行 `STOP SERVER`，无需传值。`STOP SERVER` 的更多信息请参见 [隔离节点文档](../../../../../600.manage/100.cluster-management/300.common-cluster-operations/600.isolation-a-node.md)。> 说明-f 与 -I 两者互斥，您需且仅需配置其一。 |
|  -i/--id | 否 | string | 无 | 指定前序启/停任务的 ID。仅在特殊情况下使用。当集群运维被一个或者多个前序启/停任务锁住时，可以使用 `-i` 将前序任务解锁，例如 `-i 22130706433028869`。 |
| -y/--yes  | 否  | 无  |  无   | 用于控制是否取消二次确认，无需传值。  |

## obshell cluster upgrade

使用该命令可以将 OceanBase 数据库或者 obshell 升级到指定版本。命令的详细使用场景可参见 [升级 OceanBase 集群](../210.use-case/200.cluster-management/500.upgrade-oceanbase-cluster.md)。
...
| 选项名 | 是否必选 | 数据类型 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
| -s/--server | 否 | string | 无 | 指定要从集群中删除的节点。格式为 `ip:obshell_port`，`obshell_port` 可不设置，默认为 `2886`。> 说明-s/--server 与 -z/--zone 两者互斥，您需且仅需指定其一。 |
| -z/--zone | 是 | string | 无 | 指定要从集群中删除的 Zone。> 说明-s/--server 与 -z/--zone 两者互斥，您需且仅需指定其一。 |
|-f/--force | 否 | 无    | 无   | 控制是否 kill 被删除节点上的 observer 进程，仅在删除节点时有效。当指定该选项时，会在删除节点前，kill 被删除节点上的 observer 进程。> 注意只在删除故障节点时需指定 -f/--force。  |
| -y/--yes  | 否  | 无  |  无   | 用于控制是否取消二次确认，无需传值。  |
