...
| 参数 | 类型 | 必选 | 示例值 | 描述 |
|-------|-----|-----|------|--------|
| data_base_uri | string | 否 | "file:///data/backup/data" | 数据备份文件路径。设置后数据备份文件将存储到这个路径之下。 |
| archive_base_uri | string | 否 | "file:///data/backup/clog" | 日志归档文件路径。设置后日志归档文件将存储到这个路径之下。 |
| log_archive_concurrency | int | 否 | 0 | 配置日志归档总的工作线程数量。默认值为 0，取值范围为 [0, 100]。 |
| binding | string | 否 | "optional" | 设置归档和业务的优先模式。目前支持设置为 `optional`（以用户业务优先） 和 `mandatory`（以归档优先）。如果未配置，默认为 Optional 模式。如果设置此项，则需要设置 `archive_base_uri`。 |
| ha_low_thread_score | int | 否 | 0 | 指定备份、备份清理等高可用低优先级任务的当前工作线程数。在开始备份前，可以适当提高该参数的值，建议每次将数值翻倍。默认值为 0，取值范围为 [0, 100]。 |
| piece_switch_interval | string | 否 | 1d | 配置 piece 的切换周期。如果不设置，默认为 1d，取值范围 [1d, 7d]。如果设置此项，则需要设置 `archive_base_uri`。 |
...
更新单个租户备份配置的使用和 [设置单个租户备份配置](100.configure-backup-configuration-for-a-single-tenant.md) 的区别为 `data_base_uri` 和 `archive_base_uri` 可不设置。
