...
> 说明
  非用户租户的索引使用不在统计范围，视图 DBA_INDEX_USAGE 仅展示用户租户数据。
...
> **说明**
>
> 不同租户之间的配置参数和数据汇总是相互独立的，它们之间不会相互干扰。

|        **配置项**         | **默认值** | **说明** |
|---------------------------|------------|----------|
| _iut_enable               | true       | 控制索引监控是否开启。默认值为 `true`，表示打开监控；`false` 表示关闭监控。|
| _iut_max_entries          | 30000      | 控制监控的索引表数量。默认为 30000，取值范围 [0, +∞)。> 说明此配置项可以控制索引监控视图展示的最多的记录数，每张索引表只会占用一条记录。当租户下的索引表数量超过该配置项的值时，可能出现后建的索引表被引用的信息无法被监控到，从而导致误判的情况。|
| _iut_stat_collection_type | SAMPLED    | 控制索引监控信息统计的模式。有以下两种取值可选：<ul><li><code>SAMPLED</code>：默认值，表示按采样模式监控索引的使用，采样率为 10%（采样率不可更改）。> 说明采样方式是通过索引表被引用的次数，而不是索引表的数量。在采样模式下，数据的准确性并不是绝对的。由于采样模式的目的在于筛选出一部分数据，因此可能会出现个别查询操作没有被记录下来的情况。</li><li><code>ALL</code>：表示全量监控索引表的使用，此模式虽然可以得到准确的结果，但是会有资源开销，谨慎开启。</li></ul> |
...
6. 查看视图 `DBA_INDEX_USAGE`。
    ...
    3. 再次查看视图 `DBA_INDEX_USAGE`。
       ...
       > **说明**
>
> 索引监控在收集统计信息后，通过后台定时任务把数据刷到内部表，可能最长需要等待 15 分钟，才能在视图中查询到索引使用的监控结果。
       ...
       * `LAST_USED`：索引表最后一次被使用的时间。
       ...
       更多有关视图 `DBA_INDEX_USAGE` 字段的详细信息，请参见 [DBA_INDEX_USAGE](../../../700.system-views/500.system-view-of-oracle-mode/200.dictionary-view-of-oracle-mode/8800.dba_index_usage-of-oracle-mode.md)。
