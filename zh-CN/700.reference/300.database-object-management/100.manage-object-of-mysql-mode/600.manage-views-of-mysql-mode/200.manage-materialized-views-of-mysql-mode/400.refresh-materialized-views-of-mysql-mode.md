...
> 说明
  为了使简单连接物化视图在增量刷新和实时物化视图中获得较好的性能，推荐按照以下方式为物化视图及其依赖基表创建索引：为每张表的连接键创建索引，用于提升增量更新和实时物化视图中表的连接性能。为物化视图中各基表主键列创建索引。随着物化视图中 JOIN 表数量的增加，物化视图的增量刷新性能和实时物化视图查询性能通常会有所下降。
...
> 说明
  对于 OceanBase 数据库 V4.3.5 版本，变量 mview_refresh_dop 从 V4.3.5 BP1 版本开始引入。
...
* 使用视图 [DBA_MVIEWS](../../../../700.system-views/400.system-view-of-mysql-mode/200.dictionary-view-of-mysql-mode/5000.o-dba_mviews-of-mysql-mode.md) 查询为物化视图设定的后台刷新并行度。
    ...
    > 说明
      视图 DBA_MVIEWS 查看物化视图并行度只能查看针对物化视图指定的的并行度。字段 REFRESH_DOP 为非 0 时，物化视图后台刷新任务将使用 REFRESH_DOP 对应的并行度。当 REFRESH_DOP 为 0 时，将使用设置 Global（全局）级别的 mview_refresh_dop 的值。
...
> 注意
  如果未显式指定并行度，且 mview_refresh_dop 变量为 0 或 1 时，不开启并行刷新。
...
> 说明
  可以通过视图 DBA_SCHEDULER_JOBS 查看物化视图后台刷新的任务情况。更多查看物化视图信息，参见 查询物化视图。
...
2. 设置全局 Session 变量 `mview_refresh_dop` 作为刷新并行度。
    ...
    > 注意
      如果创建物化视图时未显式指定并行度，且 mview_refresh_dop 变量为 0 或 1 时，后台刷新任务不开启并行刷新。
...
17. 再次增量刷新物化视图 `mv2`。
    ...
    > 注意
      由于 mv1 经历过全量刷新，因此直接对 mv2 进行增量刷新会报错，必须先进行一次全量刷新。
