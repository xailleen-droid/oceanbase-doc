...
* `UNIQUE`：可选，表示创建唯一多值索引。唯一多值索引，索引 Key 不可以重复。
   ...
   > **注意**
>
> 多值索引可以定义为唯一索引。如果将其定义为唯一索引，尝试插入已存在于多值索引中的值会返回重复键错误。如果已经存在重复值，尝试添加唯一的多值索引会失败。
...
后建多值索引需要首先开启配置项 [_enable_add_fulltext_index_to_existing_table](../../../800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/5980._enable_add_fulltext_index_to_existing_table.md)。
...
* `PARSER_PROPERTIES[=](parser_properties_list)`：可选项，指定分词器的属性。取值如下：
  ...
  * `ik_mode`: 表示 `IK` 分词器的分词模式。取值如下：
  ...
  > 说明
    对于 OceanBase 数据库 V4.3.5 版本，从 V4.3.5 BP1 版本开始支持指定分词器属性（PARSER_PROPERTIES）。
