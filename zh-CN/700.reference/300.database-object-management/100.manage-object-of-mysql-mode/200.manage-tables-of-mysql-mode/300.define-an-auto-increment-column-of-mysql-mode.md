...
|           系统变量          |                            说明                         |
|---------------------------|----------------------------------------------------------|
| auto_increment_cache_size | Global 级变量，用于设置自增的缓存个数，取值范围为 \[1, 100000000\]，默认值为 `1000000`。 > 说明OceanBase 数据库也支持表级设置 auto_increment_cache_size，相关语法信息请参见 CREATE TABLE 和 ALTER TABLE。|
...
2. 减小自增列的下一个取值。
   ...
   6. 再次查看表 `tbl1` 当前自增字段的取值。
      ...
      *************************** 1. row ***************************
      ...
      > 说明
        通过查询结果可以观察到，AUTO_INCREMENT 的值为 6。这是由于表中的自增列已经包含数据，且当前自增列的最大值（5）超过了尝试设置的较小值（3）。因此，尽管减小自增值的命令执行没有报错，但实际的 AUTO_INCREMENT 值自动调整为现有最大自增值的下一个数字，即 6。
