...
> 注意
  不支持二级分区类型是 Hash 或 Key 类型的场景新增二级分区。对于 OceanBase 数据库 V4.3.5 版本，从 V4.3.5 BP1 版本开始支持新增二级分区。添加的二级分区不能与表内现有的一级或二级分区重名。添加 Range 二级分区时，Range 上界要大于当前一级分区下最大分区的上界；添加多个 Range 二级分区时，上界要递增排列，即上界大的 Range 分区写在后面。添加 List 二级分区时，不能与当前一级分区下现有的二级分区值重复。添加多个二级分区的操作是原子的，要么都成功，要么都失败。
