...
| PRIMARY KEY | 指定表的主键。|
| PARTITION BY RANGE \[COLUMNS\]\(\[column_name_list\]\) | 指定预分区键，当触发自动分区时，会将填写的预分区键作为实际的分区键（若 `column_name_list` 不填写，则使用主键为分区键）。> 注意如果是指定多列分区键，需用使用 PARTITION BY RANGE COLUMNS()。对预分区键的操作限制等同于分区键。|
...
| range_partition_list | 可选项，预先创建分区位点（分区规则）。> **注意**
>
> 当预先创建分区位点时，必须指定预分区键，否则报错。|
...
> 说明
  若在使用 ALTER TABLE ... PARTITION BY ... 语句时没有定义分区规则，则会采用 Online DDL 的方式修改表的自动分区属性，并且不影响原有的分区定义。

#### 参数解释

|       **参数**       | **说明** |
|----------------------|----------|
| column_name_list     | 修改非分区表的预分区键。在使用 `ALTER TABLE` 语句修改非分区表的自动分区属性时，若 `RANGE()` 内填写了预分区键信息，则会修改 Schema 中的相应记录；如果没有填写信息，则代表保持原有设定。> 注意如果修改为多列分区键，需用使用 PARTITION BY RANGE COLUMNS()。|
...
> 说明
  可以使用视图 information_schema.TABLES 或者在系统租户中使用 oceanbase.CDB_TABLES 查看表的自动分区属性。
...
1. 关闭自动分区功能。
    ...
    > 说明
      可以使用 SHOW PARAMETERS LIKE 'enable_auto_split'; 查看自动分区功能是否开启。
...
> 说明
  对于开启自动分区的表，即使配置项 global_index_auto_split_policy 未启用，创建的全局索引仍会自动分区。
