...
| FLT_TRACE_ID | varchar(1024) | NO | 表示该条记录全链路追踪的 trace_id，如果为空，则说明没有被全链路追踪监控。该字段是Uuid，不同于 Trace。其表示形式类似于：`000600d6-a5de-038c-6c80-df07e4e79149`> 说明在 V4.3.x 版本中，该字段从 V4.3.2 版本开始引入。在 V4.2.x 版本中，该字段从 V4.2.1 版本开始引入。 |
| PL_TRACE_ID             | varchar(128)        | NO   | 当前 SQL 语句外层 PL 的 Trace ID (如果没有外层 PL 则该值 NULL)> 说明在 V4.3.x 版本中，该字段从 V4.3.2 版本开始引入。在 V4.2.x 版本中，该字段从 V4.2.2 版本开始引入。      |
| PLSQL_EXEC_TIME         | bigint(20)          | NO   | PL 执行的耗时（不包括 SQL 执行时间），单位：微秒> 说明在 V4.3.x 版本中，该字段从 V4.3.2 版本开始引入。在 V4.2.x 版本中，该字段从 V4.2.2 版本开始引入。     |
| FORMAT_SQL_ID                   | varchar(32)         | NO   |  表示该条记录通过 Format SQL text 生成的 MD5 值> 说明在 V4.3.x 版本中，该字段从 V4.3.2 版本开始引入。在 V4.2.x 版本中，该字段从 V4.2.3 版本开始引入。    |
| NETWORK_WAIT_TIME               | bigint(20) unsigned | YES  |  所有 Network 类事件的总时间，单位为微秒> 说明在 V4.3.x 版本中，该字段从 V4.3.2 版本开始引入。在 V4.2.x 版本中，该字段从 V4.2.3 版本开始引入。    |
| STMT_TYPE                       | varchar(128)        | YES  |  DML 类型会按需返回：<ul><li>DDL 类型会返回 <code>DDL_STMT</code>。</li><li>DCL 类型会返回 <code>DCL_STMT</code>。</li><li>TCL 类型会返回 <code>TCL_STMT</code>。</li><li>其他则会返回 <code>OTHER_STMT</code>。</li></ul>> 说明在 V4.3.x 版本中，该字段从 V4.3.2 版本开始引入。在 V4.2.x 版本中，该字段从 V4.2.3 版本开始引入。    |
| TOTAL_MEMSTORE_READ_ROW_COUNT   | bigint(20)          | NO   | 整个工作过程中从 MEMSTORE 读出的总行数（仅在展示 query_text 的线程中显示该变量）> 说明在 V4.3.x 版本中，该字段从 V4.3.2 版本开始引入。在 V4.2.x 版本中，该字段从 V4.2.3 版本开始引入。     |
| TOTAL_SSSTORE_READ_ROW_COUNT    | bigint(20)          | NO   | 整个工作过程中从 SSSTORE 读出的总行数（仅在展示 query_text 的线程中显示该变量）> 说明在 V4.3.x 版本中，该字段从 V4.3.2 版本开始引入。在 V4.2.x 版本中，该字段从 V4.2.3 版本开始引入。     |
| PROXY_USER                      | varchar(128)        | YES  |<ul><li>代理用户登录场景，返回代理用户的名称。</li><li>非代理用户登录场景，显示为空。</li></ul>> 说明在 V4.3.x 版本中，该字段从 V4.3.2 版本开始引入。在 V4.2.x 版本中，该字段从 V4.2.3 版本开始引入。 |
