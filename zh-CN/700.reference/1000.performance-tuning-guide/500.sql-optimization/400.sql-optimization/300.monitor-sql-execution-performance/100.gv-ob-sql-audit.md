...
| LOCK_FOR_READ_TIME      | bigint(20)          | > 注意该视图在 OceanBase 数据库 Oracle 模式中暂无字段 LOCK_FOR_READ_TIME。 |读取数据时等待锁的耗时，单位：微秒       |
| PARTITION_HIT                 | tinyint(4)            | -              | 是否命中本地 Partition                  |
| PLSQL_COMPILE_TIME            | bigint(20)            | NUMBER(38)     |  PL 编译用时。> 说明在 V4.3.x 版本中，该字段从 V4.3.5 版本开始引入。在 V4.2.x 版本中，该字段从 V4.2.5 BP2 版本开始引入。|
