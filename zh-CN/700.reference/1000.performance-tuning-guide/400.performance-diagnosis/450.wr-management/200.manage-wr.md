...
> 注意
  在手动创建快照后，定时器将会重新计时。例如在默认的间隔为 1 小时的情况下开启定期快照：12:21 定时器触发创建快照  13:21 定时器触发创建下一个快照 13:45 用户手动创建快照 14:45 定时器触发创建下一个快照 经过手动创建快照后，定时器会按照设定的间隔继续触发下一次快照的生成。
...
> 注意
  请确保删除的快照 ID 是有效的存在于系统中的快照 ID。执行删除操作后，指定范围内的快照数据将被永久删除，请谨慎操作。
