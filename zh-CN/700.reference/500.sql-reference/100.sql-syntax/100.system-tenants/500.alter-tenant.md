...
| RESOURCE_POOL_LIST       | 修改租户的资源池列表。</br>如果有多个资源池，要求多个资源池的 `UNIT_NUM` 个数一致。> 注意同一个租户的多个资源池，其对应的 ZONE_LIST 相互之间不允许有交集。 修改资源池列表时，每次仅支持增加或减少一个资源池。 |
...
* 修改租户 `tenant1` 的 Locality，增加副本数。其中 `F` 表示副本类型为全功能型副本，`B_4` 为新增的 Zone 名称。
  ...
  4. 修改租户的 Locality。
      ...
      > 注意
      修改租户的 Locality 时，对于列存副本，仅支持加减副本操作，不支持副本类型转换。即如果需要增加列存副本，则要求原 Zone 上无副本；如果需要减少列存副本，只能将列存副本所在的 Zone 改为无副本。
      在租户中增加列存副本后，如果要访问列存副本，需要额外部署 ODP，并进行一系列配置才能将连接路由到列存副本，有关列存副本的详细使用说明，参见 列存副本。
