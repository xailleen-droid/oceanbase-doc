...
| 参数项       | 描述                                                     |
|--------------|---------------------------------------------------------|
| tenant_name  | 指定待激活为主租户的备租户名。> 注意 仅系统租户在执行此命令时需要通过 TENANT tenant_name参数指定待操作的租户；普通租户只能对本租户执行此命令，故不需要指定该参数。 |
| VERIFY      | 可选项。当使用此参数时，它的作用是进行预先检验，以确认执行 `ACTIVATE STANDBY` 命令所必须的前提条件是否已经具备。 > 注意主备租户角色转换执行中存在一定失败风险。VERIFY 命令只是进行前期验证，评估满足角色转换的前提条件，有助于减少该操作失败的可能性，并不实际执行 ACTIVATE STANDBY 操作。即使 VERIFY 命令运行成功，也不能保证随后的 ACTIVATE STANDBY 操作一定会成功。|
