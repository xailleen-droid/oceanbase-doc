...
| 参数                    | 描述                                                                                                   |
|-------------------------|--------------------------------------------------------------------------------------------------------|
| all                     | 系统租户为集群中的所有用户租户开启归档模式。 > **注意**
>
> 这里的所有用户租户，指的是语句执行时集群内已存在的所有用户租户，对于语句执行之后创建的新租户，系统不会自动开启归档模式。                                                                    |
| tenant_name[, tenant_name...]    | 系统租户指定待开启归档模式的租户名，支持同时指定多个租户，不同租户之间使用英文逗号（,）分隔。> 注意仅系统租户在执行此命令时需要通过 TENANT tenant_name 子句指定待操作的租户，用户租户只能对本租户执行此命令，故不需要指定该子句。  |
