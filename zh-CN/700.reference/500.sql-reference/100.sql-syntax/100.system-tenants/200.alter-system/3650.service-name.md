...
> 说明
  通过 OCP 进行服务名管理和主备切换操作，参见 主备租户日常切换。
...
| service_name  | 指定服务名称。|
| tenant_name   | 可选项，指定待操作的租户。如果不显式指定待操作的租户，则默认租户名为当前租户。> 注意系统租户不支持 SERVICE_NAME 功能，即在系统租户下执行时该语句时，必须指定 TENANT = tenant_name 子句。|
