...
| SQL_identifier | 用于指定 SQL，格式为 `sql_id = 'xxx'`。如果不指定该参数，表示清空所有 SQL 的计划缓存。> **说明**
>
> 在系统租户 sys 下使用`SQL_identifier`参数时一定要指定`tenant_list`，在普通租户下使用则不能指定`tenant_list`参数。     |
| database_list  | 用于指定 Database。如果不指定该参数，表示清除所有计划缓存。> **说明**
>
> 在系统租户 sys 下使用`database_list`参数时一定要指定`tenant_list`，在普通租户下使用则不能指定`tenant_list`参数。  |
