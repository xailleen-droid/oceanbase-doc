...
| tenant_name           | 系统租户指定待配置备份目的端的租户名。> 注意仅系统租户在执行此命令时需要通过 TENANT = tenant_name 子句指定待操作的租户，用户租户只能对本租户执行此命令，故不能指定 TENANT = tenant_name 子句。                        |
...
> 注意
使用对象存储作为备份介质时，对象存储路径的各项参数由 & 符号进行分隔，请确保您输入的参数值中仅包含英文字母大小写、数字、/-_$+= 以及通配符。如果您输入了上述以外的其他字符，可能会导致设置失败。
...
* `delete_mode`：在备份路径后配置备份文件的清理模式，OSS、AWS S3、COS 的可选参数，与其他参数之间通过 `&` 符号连接。该参数当前支持以下两种取值：
...
* `enable_worm`：可选参数，当备份目的端为阿里云 OSS 且目的端 Bucket 配置了合规保留策略（WORM）时，需将该参数设置为 `True`。
  ...
  如果待设置的路径对应的 Bucket 设置并锁定了保留策略，而在配置备份目的端时未指定 enable_worm=true，则语句执行后，系统将会报错，提示需要设置enable_worm=true。
  enable_worm 参数一旦设置后，后续不支持变更。
  设置备份路径后，如果用户为该路径对应的 Bucket 设置并锁定了 WORM 策略，则备份任务可能会因无法修改或删除目标路径下的文件而报 -9140 的错误（错误码可通过视图 CDB_OB_BACKUP_DELETE_JOB_HISTORY（sys 租户）或 DBA_OB_BACKUP_DELETE_JOB_HISTORY（用户租户）进行查看），提示 the object is locked by worm。当发现备份任务因 WORM 策略导致执行失败后，需要用户重新配置新的备份路径，并且指定enable_worm=true。
...
* 系统租户
  ...
  * 系统租户为租户 `mysql_tenant` 配置备份路径，备份介质为 OSS，备份文件的清理模式为 `delete`，并使用 MD5 算法校验归档文件的完整性。该路径配置了 `enable_worm=true` 时，系统将按照 WORM 的规范对该路径执行写入和删除操作。

    > **注意**
>
> OceanBase 数据库当前不会替代您设置合规保留策略，请在设置备份路径前确保 OSS Bucket 已正确设置保留策略（WORM 策略）并锁定。
  ...
  * 系统租户为租户 `mysql_tenant` 配置备份路径，备份介质为 COS，备份文件的清理模式为 `delete`，通过 S3 协议访问。
    ...
    > 注意
    使用 COS 作为备份介质时，需要先设置集群级配置项 ob_storage_s3_url_encode_type，详细操作可参见 备份前准备。
...
* 用户租户
  ...
  * 租户 `mysql_tenant` 为本租户配置备份路径，备份介质为 OSS，备份文件的清理模式为 `delete`，并使用 MD5 算法校验归档文件的完整性。该路径配置了 `enable_worm=true` 时，系统将按照 WORM 的规范对该路径执行写入和删除操作。
