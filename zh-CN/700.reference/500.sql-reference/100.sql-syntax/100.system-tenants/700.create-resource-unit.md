...
| MEMORY_SIZE     | 指定内存规格，其最小值由隐藏配置项 `__min_full_resource_pool_memory` 控制，默认为 5G，为了保证系统稳定性，最小的内存规格建议为 4G。  > **注意**
>
> OceanBase 数据库从 V4.0.0.0 开始不再支持内存超卖。 |
| MAX_CPU         | 指定 CPU 规格的上限，最小值为 1，表示 1 Core。                                                                                                      |
| MIN_CPU         | 指定 CPU 规格的下限，最小值为 1，表示 1 Core，可选。</br>如果未指定，则默认其值等于 `MAX_CPU`。> 注意所有租户的 MIN_CPU 总和不能超过该节点的 CPU 总容量 CPU_CAPACITY。  |
