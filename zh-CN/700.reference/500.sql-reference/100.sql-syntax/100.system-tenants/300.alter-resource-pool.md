...
| unit_id_list                              | 减小 `unit_num` 时，使用 `DELETE UNIT` 可以删除指定的 Unit。如果不指定 `DELETE UNIT`，系统将随机删除相应个数的 Unit。 > 注意使用 DELETE UNIT 删除指定的 unit_id 时，还需要满足以下条件：  删除的 unit_id 列表中，每个 Zone 内删除的 Unit 个数必须相等。    待删除的 unit_id 列表中，每个 Zone 内删除后还剩下的 Unit 个数需要与指定的 unit_num 一致。    |
