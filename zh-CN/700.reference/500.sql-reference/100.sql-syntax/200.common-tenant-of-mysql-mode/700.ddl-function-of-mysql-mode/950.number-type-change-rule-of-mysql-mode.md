...
> 注意
  ...
  对于 Oracle 模式，不允许 Instant 转换（只修改 Schema，不变数据的转换）是指非空表不能转换；对于 MySQL 模式，不允许 Instant 转换是指需要重写数据，即使允许 Instant 转换，也不能保证所有源类型的数据都能无损地插入到目的类型中，例如定点数转 Oracle FLOAT。
