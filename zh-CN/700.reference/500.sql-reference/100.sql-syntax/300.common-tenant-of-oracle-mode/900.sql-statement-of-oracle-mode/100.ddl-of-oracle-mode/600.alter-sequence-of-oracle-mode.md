...
该语句用于更改现有序列的增量、最小值和最大值、缓存数量和行为，对应操作仅影响后续的序列号。
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p>您只能修改当前 Schema 下的序列，或者拥有该序列的 <code>ALTER</code> 权限。</p>
  </main>
...
| NOCACHE                | 更改序列的值为未预先分配。如果同时省略 `CACHE` 和 `NOCACHE`，则数据库默认缓存 20       |
| ORDER                  | 更改序列的值为按顺序生成。        |
...
* 更改序列 `seq2` 的最大值和自增步数，开启 `CYCLE` 和 `ORDER`，并指定 `NOCACHE` 以指示序列的值未预先分配。
