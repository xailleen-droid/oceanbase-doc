...
| obj_clause | 指定授权的对象。有如下取值：<ul><li>`schema_name.relation_name` </li><li>`CATALOG external_catalog_name`</li></ul>|
...
* 授予用户 `user004` 对 Catalog 的对象 `test_odps_catalog` 的 `USE CATALOG` 权限。

  ```sql
  GRANT USE CATALOG ON CATALOG test_odps_catalog TO user004;
  ```

* 授予用户 `user005` 对 `CREATE CATALOG` 和 `USE ANY CATALOG` 的系统权限。
