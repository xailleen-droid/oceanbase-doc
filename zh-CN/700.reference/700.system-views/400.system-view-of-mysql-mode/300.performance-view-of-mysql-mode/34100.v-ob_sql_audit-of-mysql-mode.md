...
| SQL_ID                  | varchar(32)         | NO             | 该 SQL 的 ID  > 说明对于 V4.3.x 版本，从 V4.3.2 版本开始，针对 CALL 语句执行的 PL 请求和通过匿名块执行的 PL 请求，该字段会展示为实际生成的 MD5 编码。 对于 V4.2.x 版本，从 V4.2.3 版本开始，针对 CALL 语句执行的 PL 请求和通过匿名块执行的 PL 请求，该字段会展示为实际生成的 MD5 编码。  |
...
| RULE_NAME | varchar(256) | NO   | 规则名称   > **说明**
>
> 该字段从 V4.1.0 版本开始引入。 |
| PARTITION_HIT | tinyint(4) | NO | <ul><li> 若获得 DAS 执行计划，命中本地 Partition 则为 1，没有命中则为 0。  </li><li>  若未获得 DAS 计划，默认为 1。 </li></ul>  > **说明**
>
> 该字段从 V4.1.0 版本开始引入。 |
| TX_INTERNAL_ROUTING | bigint(20) | NO | 表示 SQL 语句是否经过了内部路由：<ul><li>0：没有经过内部路由，即该 SQL 语句是直接在当前节点上执行的  </li><li>1：经过内部路由，即该 SQL 语句在当前节点上被路由到了其他节点上执行 </li></ul> > **说明**
>
> 该字段从 V4.1.0 版本开始引入。 |
| TX_STATE_VERSION | bigint(20) unsigned | NO | 表示 SQL 语句的事务状态版本 > **说明**
>
> 该字段从 V4.1.0 版本开始引入。 |
| FLT_TRACE_ID | varchar(1024) | NO | 表示该条记录全链路追踪的 trace_id，如果为空，则说明没有被全链路追踪监控。该字段是Uuid，不同于 Trace。其表示形式类似于：`000600d6-a5de-038c-6c80-df07e4e79149`> **说明**
>
> 该字段从 V4.2.1 版本开始引入。 |
| PL_TRACE_ID             | varchar(128)        | NO   | 当前 SQL 语句外层 PL 的 Trace ID (如果没有外层 PL 则该值 NULL) > 说明对于 V4.3.x 版本，该字段从 V4.3.0 版本开始引入。 对于 V4.2.x 版本，该字段从 V4.2.2 版本开始引入。  |
| PLSQL_EXEC_TIME         | bigint(20)          | NO   | PL 执行的耗时（不包括 SQL 执行时间），单位为微秒 > 说明对于 V4.3.x 版本，该字段从 V4.3.0 版本开始引入。 对于 V4.2.x 版本，该字段从 V4.2.2 版本开始引入。   |
| TOTAL_MEMSTORE_READ_ROW_COUNT   | bigint(20)          | NO   | 整个工作过程中从 MemStore 读出的总行数（仅在展示 query_text 的线程中显示该变量）> 说明对于 V4.3.x 版本，该字段从 V4.3.0 版本开始引入。 对于 V4.2.x 版本，该字段从 V4.2.2 版本开始引入。   |
| TOTAL_SSSTORE_READ_ROW_COUNT    | bigint(20)          | NO   | 整个工作过程中从 SSSTORE 读出的总行数（仅在展示 query_text 的线程中显示该变量）> 说明对于 V4.3.x 版本，该字段从 V4.3.0 版本开始引入。 对于 V4.2.x 版本，该字段从 V4.2.2 版本开始引入。  |
| PROXY_USER                      | varchar(128)        | NO  |<ul><li>代理用户登录场景，返回代理用户的名称。</li><li>非代理用户登录场景，显示为空。</li></ul> > 说明对于 V4.3.x 版本，该字段从 V4.3.2 版本开始引入。 对于 V4.2.x 版本，该字段从 V4.2.3 版本开始引入。 |
| SEQ_NUM                         | bigint(20)          | NO   | 事务中的语句序号> 说明对于 V4.3.x 版本，该字段从 V4.3.5 版本开始引入。 对于 V4.2.x 版本，该字段从 V4.2.5 版本开始引入。    |
| NETWORK_WAIT_TIME               | bigint(20) unsigned | YES  | 所有 Network 类事件的总时间，单位为微秒> 说明对于 V4.3.x 版本，该字段从 V4.3.5 版本开始引入。 对于 V4.2.x 版本，该字段从 V4.2.3 版本开始引入。     |
| PLSQL_COMPILE_TIME              | bigint(20)          | NO   |  PL 编译用时。> 说明在 V4.3.x 版本中，该字段从 V4.3.5 版本开始引入。在 V4.2.x 版本中，该字段从 V4.2.5 BP2 版本开始引入。|
