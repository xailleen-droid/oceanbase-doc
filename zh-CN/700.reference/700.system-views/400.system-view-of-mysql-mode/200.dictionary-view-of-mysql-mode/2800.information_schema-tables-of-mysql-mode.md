...
| DATA_LENGTH     | bigint(21) unsigned | NO | 数据长度，即表占据的存储空间，按宏块大小计算，单位为 Byte。> 说明p>对于 V4.3.x 版本，该字段从 V4.3.5 版本开始生效。 |
| MAX_DATA_LENGTH | bigint(0) unsigned  | NO | 最大数据长度。 |
| INDEX_LENGTH    | bigint(0) unsigned  | NO | 索引长度，即非主键索引占据的存储空间，按宏块大小计算，单位为 Byte。> 说明p>对于 V4.3.x 版本，该字段从 V4.3.5 版本开始生效。 |
...
| AUTO_SPLIT    | varchar(16) | NO | 表示表是否开启了自动分区。取值如下：<ul><li>`FALSE`（默认值）：表示表关闭了自动分区。</li><li>`TRUE`：表示表开启了自动分区。</li></ul> > **说明**
>
> 该字段从 V4.3.4 版本开始引入。|
| AUTO_SPLIT_TABLET_SIZE | bigint(20) unsigned | NO | 表示自动分区分裂阈值。当字段 `AUTO_SPLIT` 取值为 `FALSE` 时，`AUTO_SPLIT_TABLET_SIZE` 取值为 0。 > **说明**
>
> 该字段从 V4.3.4 版本开始引入。|
| ORGANIZATION    | varchar(12)         | NO | 表示表组织模式。取值如下：<ul><li>`HEAP`：表示为堆组织表模式。</li><li>`INDEX`：表示为索引组织表模式。</li></ul> > **说明**
>
> 对于 OceanBase 数据库 V4.3.5 版本，该字段从 V4.3.5 BP1 版本开始支持引入。|
