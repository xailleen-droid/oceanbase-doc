...
| AUTO_SPLIT                | VARCHAR2(16)   | NO         | 表示表是否开启了自动分区。取值如下：<ul><li>`FALSE`（默认值）：表示表关闭了自动分区。</li><li>`TRUE`：表示表开启了自动分区。</li></ul> > **说明**
>
> 该字段从 V4.3.4 版本开始引入。|
| AUTO_SPLIT_TABLET_SIZE    | VARCHAR2(128)  | NO         | 表示自动分区分裂阈值。当字段 `AUTO_SPLIT` 取值为 `FALSE` 时，`AUTO_SPLIT_TABLET_SIZE` 取值为 0。 > **说明**
>
> 该字段从 V4.3.4 版本开始引入。|
