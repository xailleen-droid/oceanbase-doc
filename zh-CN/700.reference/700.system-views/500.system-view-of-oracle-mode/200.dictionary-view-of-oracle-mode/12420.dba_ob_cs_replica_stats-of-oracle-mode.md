...
> **说明**
>
> 对于 V4.3.5 版本，该视图从 V4.3.5 BP1 版本开始引入。
...
|---------------------------|---------------|---------------------|-------------------------------------|
| SVR_IP                    | VARCHAR2(46)  | NO   | C 副本所在的服务器 IP |
| SVR_PORT                  | NUMBER(38)    | NO   | C 副本所在的服务器的端口号  |
| LS_ID                     | NUMBER(38)    | NO   | 日志流 ID  |
| TOTAL_TABLET_CNT          | NUMBER(38)    | NO   | 需要转为列存的分区数量  |
| AVAILABLE_TABLET_CNT      | NUMBER        | NO   | 可用的分区数量    |
| TOTAL_MACRO_BLOCK_CNT     | NUMBER        | NO   | 总的基线宏块数（不考虑多版本）  |
| AVAILABLE_MACRO_BLOCK_CNT | NUMBER        | NO   | 可用的基线宏块数（不考虑多版本）   |
| AVAILABLE                 | VARCHAR2(5)   | NO   | 日志流是否已经完全可用：<ul><li>`TRUE`：可用。当该日志流上的所有 Tablet 均可用时，该日志流才完全可用。</li> <li>`FALSE`：不可用</li></ul>  |
