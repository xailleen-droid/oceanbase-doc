...
视图 `DBA_AUDIT_OBJECT` 显示数据库中所有对象的审计跟踪记录。
...
```shell
obclient [SYS]> SELECT * FROM SYS.DBA_AUDIT_OBJECT WHERE ROWNUM =1\G
```

## 相关文档

查询当前用户可访问的对象的审计跟踪记录：[USER_AUDIT_OBJECT](25100.user_audit_object-of-oracle-mode.md)
