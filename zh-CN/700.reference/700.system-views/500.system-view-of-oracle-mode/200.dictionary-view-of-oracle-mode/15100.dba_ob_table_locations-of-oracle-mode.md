...
| DUPLICATE_SCOPE | VARCHAR2(7) | NO | 表明这个表是否为复制表，取值为：<ul><li>`CLUSTER`：复制表  </li><li>`NONE`：普通表 </li></ul>> **说明**
>
> 该字段从 V4.2.0 版本开始引入。 |
| DUPLICATE_READ_CONSISTENCY | varchar(6) | NO | 展示复制表备副本的读一致性约束，取值如下：<ul><li>`NONE`：表示非复制表，该属性无意义。</li><li>`STRONG`：表示复制表备副本支持强一致性读。</li></ul> > **说明**
>
> 该字段从 V4.3.4 版本开始引入。|
| OBJECT_ID | NUMBER(38) | NO | 分区对应的对象 ID> **说明**
>
> 该字段从 V4.2.0 版本开始引入。 |
| TABLEGROUP_NAME | VARCHAR2(128) | NO | 表对应 Table Group 的名称> **说明**
>
> 该字段从 V4.2.0 版本开始引入。 |
| TABLEGROUP_ID | NUMBER(38) | NO | 表对应 Table Group 的 ID> **说明**
>
> 该字段从 V4.2.0 版本开始引入。 |
| SHARDING | VARCHAR2(10) | NO | 表对应 Table Group 的 Sharding 属性（均衡模式）> **说明**
>
> 该字段从 V4.2.0 版本开始引入。 |
