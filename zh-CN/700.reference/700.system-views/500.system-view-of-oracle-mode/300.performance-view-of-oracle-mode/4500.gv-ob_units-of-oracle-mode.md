...
| ZONE_TYPE        | VARCHAR2(128) | NO         | Unit 所在 Zone 的 Zone 类型，取值如下：<ul><li>`NULL`：表示该列值无效，OBServer 节点重启的场景下会短暂出现一段时间的无效值。</li> <li>`ReadWrite`：读写 Zone</li> <li>`Encryption`：加密 Zone</li></ul> > **说明**
>
> 该字段从 V4.2.0 版本开始引入。 |
| REGION           | VARCHAR2(128) | NO         | Unit 所在 Zone 所属的 Region 名。未设置时，默认为 `default_region`。取值为 `NULL` 时，表示该列值无效，OBServer 节点重启的场景下会短暂出现一段时间的无效值。> **说明**
>
> 该字段从 V4.2.0 版本开始引入。  |
...
| MAX_NET_BANDWIDTH| NUMBER(38)   | YES        | Unit 的最大网络带宽，单位为字节  > **说明**
>
> 该字段从 V4.3.4 版本开始引入。 |
| NET_BANDWIDTH_WEIGHT |NUMBER(38)| YES        | Unit 的网络带宽的权重值 > **说明**
>
> 该字段从 V4.3.4 版本开始引入。  |
| LOG_DISK_SIZE    | NUMBER(38)   | NO         | 日志盘大小，单位为字节                |
| LOG_DISK_IN_USE  | NUMBER(38)   | NO         | 日志盘已使用大小，单位为字节                                                               |
| DATA_DISK_SIZE   | NUMBER(38)   | YES        | <ul><li>表示 Unit 能够使用的本地缓存大小，单位为字节，仅适用于共享存储（Shared-Storage，SS）模式。</li> <li>无共享（Shared-Nothing，SN） 模式下，该字段值为 `NULL`，表示未定义，允许竞争使用其他租户的磁盘空间。</li></ul> > **说明**
>
> 该字段从 V4.3.4 版本开始引入。     |
