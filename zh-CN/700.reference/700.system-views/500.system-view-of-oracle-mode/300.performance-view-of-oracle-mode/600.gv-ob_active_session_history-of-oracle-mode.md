...
| GROUP_ID         | NUMBER | YES  | 所属资源组 ID。     |
| PLAN_HASH                   | NUMBER          | YES  | 当前执行 SQL 命令对应 Plan Hash。     |
| THREAD_ID                   | NUMBER          | YES  | 当前活跃会话所在的线程 ID。    |
| STMT_TYPE                   | NUMBER          | YES  | 当前活跃会话的 SQL 类型。     |
...
| ACTION | VARCHAR2(64) | YES | 会话在采样时记录的 ACTION 值，通过 `DBMS_APPLICATION_INFO.SET_ACTION` 包设置。 |
| CLIENT_ID | VARCHAR2(64) | YES | 会话在采样时记录的 CLIENT_ID 值，通过 `DBMS_SESSION.set_identifier` 包设置。 |
...
| TX_ID                       | NUMBER          | YES  | 当前事务 ID     |
| BLOCKING_SESSION_ID         | NUMBER          | YES  | 如果当前会话被阻塞，显示阻塞该会话的会话 ID，目前仅在锁冲突场景生效，展示持有锁的会话 ID      |
| TABLET_ID                   | NUMBER          | YES  | 当前 SQL 正在处理的 Tablet 的 ID     |
| PROXY_SID                   | NUMBER          | YES  | 代理会话 ID     |
| DELTA_READ_IO_REQUESTS      | NUMBER          | NO | 两次采样之间读的次数。<main id="notice" type='explain'><h4>说明</h4><ul><li>对于 V4.3.x 版本，该字段从 V4.3.5 BP2 版本开始引入。</li> <li>对于 V4.2.x 版本，该字段从 V4.2.5 BP3 版本开始引入。</li></ul></main>     |
| DELTA_READ_IO_BYTES         | NUMBER          | NO | 两次采样之间读的文件累计大小。<main id="notice" type='explain'><h4>说明</h4><ul><li>对于 V4.3.x 版本，该字段从 V4.3.5  BP2 版本开始引入。</li> <li>对于 V4.2.x 版本，该字段从 V4.2.5 BP3 版本开始引入。</li></ul></main>     |
| DELTA_WRITE_IO_REQUESTS     | NUMBER          | NO | 两次采样之间写的次数。<main id="notice" type='explain'><h4>说明</h4><ul><li>对于 V4.3.x 版本，该字段从 V4.3.5 BP2 版本开始引入。</li> <li>对于 V4.2.x 版本，该字段从 V4.2.5 BP3 版本开始引入。</li></ul></main>     |
| DELTA_WRITE_IO_BYTES        | NUMBER          | NO | 两次采样之间写的文件累计大小。<main id="notice" type='explain'><h4>说明</h4><ul><li>对于 V4.3.x 版本，该字段从 V4.3.5 BP2 版本开始引入。</li> <li>对于 V4.2.x 版本，该字段从 V4.2.5 BP3 版本开始引入。</li></ul></main>     |
