...
视图 `V$OB_ACTIVE_SESSION_HISTORY` 用于展示本租户下当前 OBServer 节点的活跃会话历史记录。
...
| TX_ID                       | NUMBER          | NO  | 当前事务 ID     |
| BLOCKING_SESSION_ID         | NUMBER          | NO  | 如果当前会话被阻塞，显示阻塞该会话的会话 ID，目前仅在锁冲突场景生效，展示持有锁的会话 ID      |
| TABLET_ID                   | NUMBER          | NO  | 当前 SQL 正在处理的 Tablet 的 ID     |
| PROXY_SID                   | NUMBER          | NO  | 代理会话 ID     |
| TX_ID                       | NUMBER          | NO | 当前事务 ID     |
