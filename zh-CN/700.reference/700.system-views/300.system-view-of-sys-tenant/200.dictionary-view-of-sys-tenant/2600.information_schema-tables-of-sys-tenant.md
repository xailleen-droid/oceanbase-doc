...
| DATA_LENGTH     | bigint(21) unsigned | NO | 数据长度，即表占据的存储空间，按宏块大小计算，单位为 Byte。> **说明**
>
> 对于 V4.3.x 版本，该字段从 V4.3.5 版本开始不再展示为 `NULL`。 |
| MAX_DATA_LENGTH | bigint(0) unsigned  | NO | 最大数据长度。 |
| INDEX_LENGTH    | bigint(0) unsigned  | NO | 索引长度，即非主键索引占据的存储空间，按宏块大小计算，单位为 Byte。> **说明**
>
> 对于 V4.3.x 版本，该字段从 V4.3.5 版本开始不再展示为 `NULL`。 |
