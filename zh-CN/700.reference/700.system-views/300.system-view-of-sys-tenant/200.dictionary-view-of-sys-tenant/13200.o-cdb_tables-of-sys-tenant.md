...
| IOT_TYPE                  | varchar(12)   | NO         | 表示表组织模式。取值如下：<ul><li>`NULL`：表示堆组织表模式。</li><li>`IOT`：表示为索引组织表模式。</li></ul> > **说明**
>
> 对于 OceanBase 数据库 V4.3.5 版本，该字段从 V4.3.5 BP1 版本开始支持。|
...
>
> 该字段从 V4.3.4 版本开始引入。|
| AUTO_SPLIT_TABLET_SIZE    | bigint(20)    | NO         | 表示自动分区分裂阈值。当字段 `AUTO_SPLIT` 取值为 `FALSE` 时，`AUTO_SPLIT_TABLET_SIZE` 取值为 0。 > **说明**
>
> 该字段从 V4.3.4 版本开始引入。|
