...
| DATA_PROGRESS                 | decimal(26,2)  | NO              | 数据备份的备份进度。计算方法为 `完成的宏块个数 / 总宏块个数`，结果保留两位小数 。例如，87.65 表示当前进度为 87.65%。  > 说明对于 V4.3.x 版本，该字段从 V4.3.1 版本开始引入。 对于 V4.2.x 其他版本，该字段从 V4.2.1 BP3 版本开始引入。    |
| LOG_FILE_COUNT                | bigint(20)     | NO              | 归档 clog 文件的总数   > **说明**
>
> 该字段从 V4.3.1 版本开始引入。     |
| FINISH_LOG_FILE_COUNT         | bigint(20)     | NO              | 完成备份的归档 clog 文件的个数    > **说明**
>
> 该字段从 V4.3.1 版本开始引入。    |
| LOG_PROGRESS                  | decimal(26,2)  | NO              | 归档 clog 文件的备份进度。计算方法为 `已完成备份的归档 clog 文件的个数 / 归档 clog 文件的总数`，结果保留两位小数。例如，87.65 表示当前进度为 87.65%。  > **说明**
>
> 该字段从 V4.3.1 版本开始引入。       |
