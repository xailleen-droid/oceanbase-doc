...
| LEVEL          | bigint(20)          | NO         | 表示该 Session 的全链路追踪监控级别。例如， 1 表示 level 为 1 的诊断信息> **说明**
>
> 该字段从 V4.2.1 版本开始引入。 |
| SAMPLE_PERCENTAGE | bigint(20)       | NO         | 表示该 Session 的全链路追踪采样频率。 例如， 50 表示以 50% 的频率进行诊断信息的采样> **说明**
>
> 该字段从 V4.2.1 版本开始引入。 |
...
>
> 该字段从 V4.2.1 版本开始引入。 |
| LB_VID           | bigint(20)        | YES         | 如果是公有云环境上通过负载均衡直接连接数据库，则本列显示的是负载均衡服务的 VPC ID。否则显示为 NULL> **说明**
>
> 该字段从 V4.2.1 版本开始引入。 |
| LB_VIP           | varchar(46)       | YES         | 如果是公有云环境上通过负载均衡直接连接数据库，则本列显示的是客户端连接负载均衡服务的 IP。否则显示为 NULL> **说明**
>
> 该字段从 V4.2.1 版本开始引入。 |
| LB_VPORT         | bigint(20)        | YES         | 如果是公有云环境上通过负载均衡直接连接数据库，则本列显示的是客户端连接负载均衡服务的端口。否则显示为 NULL> **说明**
>
> 该字段从 V4.2.1 版本开始引入。 |
| IN_BYTES         | bigint(20)        | NO          | 表示该会话的流入流量，单位为 byte  > **说明**
>
> 该字段从 V4.2.1 版本开始引入。 |
| OUT_BYTES        | bigint(20)        | NO          | 表示该会话的流出流量，单位为 byte  > **说明**
>
> 该字段从 V4.2.1 版本开始引入。 |
| USER_CLIENT_PORT  | bigint(20)          | NO   |   客户端地址信息 > 说明对于 V4.3.x 版本，该字段从 V4.3.0 版本开始引入。 对于 V4.2.x 版本，该字段从 V4.2.3 版本开始引入。  |
...
| SERVICE_NAME     | varchar(64)       | YES         | 该 Session 由哪个服务名创建，值为 `NULL` 表示该 Session 不是由服务名创建。 > 说明对于 V4.3.x 版本，该字段从 V4.3.1 版本开始引入。 对于 V4.2.1 版本，该字段从 V4.2.1 BP9 版本开始引入；对于 V4.2.x 其他版本，该字段从 V4.2.4 版本开始引入。 |
| TOTAL_CPU_TIME   | bigint(21)        | NO          | 当前命令执行 CPU 使用时间，单位为秒 > 说明对于 V4.3.x 版本，该字段从 V4.3.1 版本开始引入。 对于 V4.2.1 版本，该字段从 V4.2.1 BP5 版本开始引入。 |
| TOP_INFO         | varchar(262143)   | YES         | 展示执行中 SQL 所在的顶层 PL 语句信息。 > 说明对于 V4.3.x 版本，该字段从 V4.3.3 版本开始引入。 对于 V4.2.1 版本，该字段从 V4.2.1 BP9 版本开始引入。 |
| MEMORY_USAGE     | bigint(20)        | YES         | 单条 SQL 语句当前占用的内存大小，单位为字节   > **说明**
>
