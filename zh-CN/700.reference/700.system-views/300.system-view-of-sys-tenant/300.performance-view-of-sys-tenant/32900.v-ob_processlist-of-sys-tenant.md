...
>
> 该字段从 V4.2.1 版本开始引入。 |
| LB_VID            | bigint(20)       | NO         | 如果是公有云环境上通过负载均衡直接连接数据库，则本列显示的是负载均衡服务的 VPC ID。否则显示为 NULL> **说明**
>
> 该字段从 V4.2.1 版本开始引入。 |
| LB_VIP            | varchar(46)      | NO         | 如果是公有云环境上通过负载均衡直接连接数据库，则本列显示的是客户端连接负载均衡服务的 IP。否则显示为 NULL> **说明**
>
> 该字段从 V4.2.1 版本开始引入。 |
| LB_VPORT          | bigint(20)       | NO         | 如果是公有云环境上通过负载均衡直接连接数据库，则本列显示的是客户端连接负载均衡服务的端口。否则显示为 NULL> **说明**
>
> 该字段从 V4.2.1 版本开始引入。 |
...
| PROXY_USER        | varchar(128)        | NO  |<ul><li>代理用户登录场景，返回代理用户的名称。</li><li>非代理用户登录场景，显示为空。</li></ul>> 说明在 V4.3.x 版本中，该字段从 V4.3.2 版本开始引入。在 V4.2.x 版本中，该字段从 V4.2.3 版本开始引入。 |
...
| MEMORY_USAGE     | bigint(20)        | NO          | 单条 SQL 语句当前占用的内存大小，单位为字节   > **说明**
>
